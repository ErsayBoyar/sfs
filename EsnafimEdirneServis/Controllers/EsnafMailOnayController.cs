...
{
    public class EsnafMailOnayController : ApiController
    {
        ...
        Error myError;
        [CustomAuthorization]
        public async Task<IHttpActionResult> Put(int esnafId, Esnaf esn)
        {

            EsnafimEdirneEntities db = new EsnafimEdirneEntities();
            Esnaf db_esnaf = new Esnaf();
            var varmi = db.Esnaf.Where(a => a.Id == esnafId).SingleOrDefault();
            if (varmi != null)
            {
                try
                {
