...
{
    public class EsnafOnlineDurumController : ApiController
    {
        ...
        Error myError;
        [CustomAuthorization]
        public async Task<IHttpActionResult> Put(Int32 id, bool durum, Esnaf esn)
       
        {
            EsnafimEdirneEntities db = new EsnafimEdirneEntities();

            var list = db.Esnaf.Where(x => x.Id == id).SingleOrDefault();
            if (list != null)
            {
                list.Online = Convert.ToBoolean(durum);
                db.SaveChanges();
            }
            else
            {
